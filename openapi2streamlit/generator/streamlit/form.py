...
from openapi2streamlit.generator.streamlit.utils import (
    serialize_path_to_import, generate_streamlit_fields, generate_creation_file_info,
    generate_streamlit_component_folder, add_tabulation_to_multiline_string
    )
...
def generate_streamlit_form_component(component_data: dict, output_dir="output/"):
    """Generates a Streamlit file for a given endpoint"""
    
    env = Environment(loader=FileSystemLoader("openapi2streamlit/templates/streamlit/components/"))
    ...
    func_endpoint = component_data.get('endpoint', {})
