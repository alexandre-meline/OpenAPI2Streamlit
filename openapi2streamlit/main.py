...
from openapi2streamlit.generator.streamlit.form import (
    generate_streamlit_form_component
    )
from openapi2streamlit.generator.streamlit.api import (
    generate_streamlit_api_component
    )


def main():
    parser = argparse.ArgumentParser(
        description="Streamlit Component Generator from OpenAPI"
        )

    parser.add_argument("openapi_file",
                        help="OpenAPI file path (JSON or YAML)")
    ...
    for endpoint, details in endpoints.items():
        ...
        component_data = generate_streamlit_api_component(
            endpoint_datas, args.output
            )
